...
if (isset($_POST['user_userupdate'])) {
    $update_id = $makh;
    $diachi1 = $_POST['user_useraddress'];
    $tenkh1 = $_POST['user_username'];
    $email1 = $_POST['user_email'];
    $phone1 = $_POST['user_userphone'];
    $image = $_FILES['user_image']['name'];
    $image_tmp = $_FILES['user_image']['tmp_name'];
    ...
    try {
        ...
        // Gán giá trị tham số
        $stmt->bindParam(':ten_khach_hang', $tenkh1, PDO::PARAM_STR);
        $stmt->bindParam(':email', $email1, PDO::PARAM_STR);
        $stmt->bindParam(':so_dien_thoai', $phone1, PDO::PARAM_STR);
        $stmt->bindParam(':dia_chi', $diachi1, PDO::PARAM_STR);
